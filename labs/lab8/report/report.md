...
```
// Параметры модели
N = 40;       // тыс. потребителей
p_cr = 12;    // тыс. единиц
q = 1;
tau1 = 20; tau2 = 30;
p1 = 8; p2 = 5; // тыс. единиц
M10 = 5; M20 = 4.5; // млн единиц

// Вычисление коэффициентов
a1 = p_cr / (tau1^2 * p1^2 * N * q); // ≈ 1.171875e-5
a2 = p_cr / (tau2^2 * p2^2 * N * q); // ≈ 1.333333e-5
b = p_cr / (tau1^2 * p1^2 * tau2^2 * p2^2 * N * q); // ≈ 5.208333e-10
c1 = (p_cr - p1) / (tau1 * p1); // = 0.025
c2 = (p_cr - p2) / (tau2 * p2); // ≈ 0.0466667
...
function du = competition_model2(theta, u)
    M1 = u(1); M2 = u(2);
    du = zeros(2,1);
    du(1) = M1 - ((b/c1) + 0.0002)*M1*M2 - (a1/c1)*M1^2;
    du(2) = (c2/c1)*M2 - (b/c1)*M1*M2 - (a2/c1)*M2^2;
endfunction

// Решение
u = ode(u0, t0, theta, competition_model2);
M1 = u(1,:); M2 = u(2,:);

// График
scf(2);
plot(theta, M1, 'b-', 'LineWidth', 2);
plot(theta, M2, 'r-', 'LineWidth', 2);
xlabel('Нормированное время, θ');
ylabel('Объем продаж, млн единиц');
title('Случай 2: Рыночные + социально-психологические факторы');
legend(['M1(θ) - Фирма 1'; 'M2(θ) - Фирма 2'
